...
function postman_tests {
  ...
  if [[ $count == 0 ]]; then
    ...
    java -Dorg.slf4j.simpleLogger.log.com.tco=error -jar -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005 ${BUILD_DIRECTORY_PREFIX}/target/server-local.jar ${SERVER_PORT} &
    bg_pid=$!

    #Sleep for 2 seconds to give time for the server to run
    sleep 2
